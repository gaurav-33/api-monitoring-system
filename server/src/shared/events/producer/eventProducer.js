...
export class EventProducer {
    ...
    _incrementMatric(metric) {
        this._matrics[metric] = (this._matrics[metric] || 0) + 1;
    }
    ...
    async publishApiHit(eventData, opts = {}) {
        ...
        if (!this._circuitBreaker.allowRequest()) {
            this._logger.info('[EventProducer] publish rejected publish', {
                eventId: eventData.eventId,
        ...
        while (true) {
            try {
                ...
                this._incrementMatric('published');
                ...
                if (!canRetry) {
                    ...
                    this._incrementMatric('failed');
                    if (!this._retry.shouldRetry(attempt)) {
                        this._incrementMatric('retriesExhausted')
                    }
                    throw error
                }
    ...
    async _publish(eventData, { correlationId, attempt }) {
        ...
        return new Promise((resolve, reject) => {
            console.log('DEBUG channel keys:', Object.keys(channel));
            console.log('DEBUG channel constructor:', channel?.constructor?.name);
            console.log('DEBUG typeof publish:', typeof channel?.publish);
