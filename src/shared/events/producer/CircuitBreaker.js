...
export class CircuitBreaker {
    ...
    _transitionTo(newState) {
        ...
        if (newState === CircuitState.HALF_OPEN) {
            this._halfOpenAttempts = 0;
            this._halfOpenSuccesses = 0;
            this.logger.info(`[CircuitBreaker] ${prev} => ${CircuitState.HALF_OPEN}`)
        }
    }
    ...
    _openCircuit() {
        ...
        this.logger.error(`[CircuitBreaker] ${CircuitState.OPEN}`, {
    ...
    onSuccess() {
        ...
        if (this._state === CircuitState.HALF_OPEN) {
            ...
            if (this._halfOpenSuccesses >= this.halfOpenMaxAttempts) {
                this._reset();
                this.logger.info(`[CircuitBreaker] reset to ${CircuitState.CLOSED} after successful half-open probes`)
            }
            return;
        }
        if (this._failures > 0) {
    ...
    onFailure() {
        ...
        if (this._state === CircuitState.HALF_OPEN) {
            this._openCircuit();
            this.logger.info(`[CircuitBreaker] ${CircuitState.HALF_OPEN} failed, reopening  cricuit`);
            return;
        }
        this._failures++;
        this._lastFailureTime = Date.now();
